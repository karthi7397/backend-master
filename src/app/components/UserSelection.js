...
export default function UserSelection({ selectUser }) {
  ...
  useEffect(() => {
    ...
    const storedResults = localStorage.getItem('backend-master_quizResults');
    if (storedResults) {
  ...
  const addUser = (name) => {
    ...
    localStorage.setItem('backend-master_quizUserId', newUserId);
    ...
    localStorage.setItem('backend-master_quizResults', JSON.stringify(updatedUsers));
