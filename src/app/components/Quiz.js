...
export default function Quiz({ quizType, user, resetQuiz }) {
  ...
  const finishQuiz = () => {
    ...
    const currentUserId = localStorage.getItem('backend-master_quizUserId');

    if (currentUserId) {
      // Get existing quiz results from local storage
      const storedResults = JSON.parse(localStorage.getItem('backend-master_quizResults') || '[]');
      ...
      localStorage.setItem('backend-master_quizResults', JSON.stringify(updatedResults));
    }
  };
